...
| 邰天成 | 2019.4.19 | 改善了一下格式以及上传了Order部分的内容V | V0.3.1 |
| 陈立 | 2019.4.11 | 增加FilmArrange部分 | V0.3.2 |
| 潘祺阳 | 2019.4.20 |增加了VIPcard及salesPromotion部分| V0.3.3 |
| 翁溥鸿 | 2019.4.20 |增加了user及film部分| V0.3.4 |
| 邰天成 | 2019.4.20 |最后整理| V1.0 |
...
![](http://47.111.9.220:8080/photos/bao/0.png)

![](http://47.111.9.220:8080/photos/bao/1.png)
...
![](http://47.111.9.220:8080/photos/bao/3.png)
...
![](http://47.111.9.220:8080/photos/bao/4.png)
...
![](http://47.111.9.220:8080/photos/bao/5.png)
...
根据需求，系统存在19个用户界面：登录界面、注册界面、观众用户界面、影院员工界面、管理员界面、搜索电影界面、查看电影列表界面、查看电影详情界面、查看排片界面、排片管理界面、统计数据界面、发布优惠活动界面、查看想看人数界面、上架电影界面、查看会员卡界面、会员卡充值界面、购买会员卡界面、购票界面、支付界面。界面跳转如下图所示。![](http://47.111.9.220:8080/photos/bao/界面.png)

##### 5.2.1.1类图

![](http://47.111.9.220:8080/photos/bao/类图.png)
...
| User.register        | 前置条件 | password符合输入规则                                         |
|                      | 后置条件 | 查找是否存在相应的User，根据输入的id返回注册的结果           |
| **需要的服务（需接口）**                                |                                     ||
| UserDataService.insert(UserPO po)                     | 插入UserPO单一持久化对象           ||
| UserDataService.delete(long userID)                   | 删除UserPO单一持久化对象           ||
| UserDataService.findByUserID(long userID)              | 根据用户ID查找UserPO单一持久化对象 ||
| UserDataService.update(UserPO po)                     | 更新UserPO单一持久化对象           ||
##### 5.3.2.2 Filmbl模块的接口规范如下表所示。
**表5-3-2-2 Filmbl模块的接口规范**
...
| Film.addFilm | 前置条件 | 电影所有信息符合输入规则，人员权限等级为管理员                       |
|                      | 后置条件 | 查找是否存在相应的Film，根据输入的电信信息返回上架电影的结果 |
|                      | 语法     | public Map showAllFilmWanted() |
| Film.showAllFilmWanted                    | 前置条件 | 人员权限等级为管理员                                         |
|                      | 后置条件 | 以图表的形式返回所有电影想看人数信息                         |
| | 语法                                       | public Map showOneFilmWanted()                               |
| Film.showOneFilmWanted | 前置条件                                   | 人员权限等级为管理员                                         |
| | 后置条件                                   | 系统以折线图的形式显示该电影自发布时间到当前日期为止每天想看人数的变化趋势 |
| | 语法                                       | public List showFilmList()                                   |
| Film.showFilmList | 前置条件                                   | 无                                                           |
| | 后置条件                                   | 系统返回所有上架电影的列表                                   |
| | 语法                                       | public FilmPO showOneFilmDetails(long filmid)                |
| Film.showOneFilmDetails | 前置条件                                   | 无                                                           |
| | 后置条件                                   | 系统返回所选电影的详情                                       |
| | 语法                                       | public void addFilmMark(long filmid)                         |
| Film.addFilmMark | 前置条件                                   | 无                                                           |
| | 后置条件                                   | 系统更改所选电影的想看人数信息以及修改用户是否标记此电影的状态 |
| | 语法                                       | public List searchFilm(String key)                           |
| Film.searchFilm | 前置条件                                   | 输入的搜索信息不为空                                         |
| | 后置条件                                   | 系统返回模糊搜索后相关的电影列表                             |
| **需要的服务（需接口）**                                |                                     ||
| FilmDataService.insert(FilmPO po)             | 插入FilmPO单一持久化对象       ||
| FilmDataService.delete(long userID)               | 删除FilmPO单一持久化对象       ||
| FilmDataService.findByFilmID(long filmID)  | 根据FilmID查找FilmPO单一持久化对象 ||
| FilmDataService.update(FilmPO po)             | 更新FilmPO单一持久化对象       ||
| FilmDataService.findByKey(String key) | 根据关键字模糊搜索查找FilmPO多个持久化对象 ||
##### 5.3.2.3 Orderbl模块的接口规范如下表所示。

​                                                                      表5-3-2-3 Orderbl模块的接口规范  
...
##### 5.3.2.5 VIPcardbl模块的接口规范如下表所示

  表5-3-2-5 VIPcard模块的接口规范
...
##### 5.3.2.6 SalesPromotion模块的接口规范如下表所示

  表5-3-2-6 SalesPromotion模块的接口规范
...
#### 5.4.2接口规范
##### 5.4.2.1UserDataService模块的接口规范如下表所示
​                                                                    表5-4-2-1 UserDataService模块的接口规范  

| 提供的服务（供接口）    |          |                                   |
| ----------------------- | -------- | --------------------------------- |
|                         | 语法     | public UserPO find(long userID) |
| UserDataService.find   | 前置条件 | 无                                |
|                         | 后置条件 | 按ID进行查找返回相应的OrderPO结果 |
|                         | 语法     | public void insert(UserPO po)    |
| UserDataService.insert | 前置条件 | 同样ID的po在Mapper中不存在        |
|                         | 后置条件 | 在数据库中添加一个po记录          |
|                         | 语法     | public void delete(UserPO po)    |
| UserDataService.delete | 前置条件 | 在数据库中存在同样ID的po          |
|                         | 后置条件 | 删除一个po                        |
|                         | 语法     | public void update(UserPO po)    |
| UserDataService.update | 前置条件 | 数据库中存在同样ID的po            |
|                         | 后置条件 | 更新一个po                        |
|                         | 语法     | public init()                     |
...
|                         | 后置条件 | 结束持久化数据库的使用            |
##### 5.4.2.2FilmDataService模块的接口规范如下表所示
​                                                                    表5-4-2-2FilmDataService模块的接口规范  

| 提供的服务（供接口）    |          |                                   |
| ----------------------- | -------- | --------------------------------- |
|                         | 语法     | public FilmPO find(long filmID) |
| FilmDataService.findByFilmID   | 前置条件 | 无                                |
|                         | 后置条件 | 按ID进行查找返回相应的FilmrPO结果 |
|                         | 语法     | public void insert(FilmPO po)    |
| FilmDataService.findByKey   | 前置条件 | 无                                |
|                         | 后置条件 | 按关键字进行查找返回相应的FilmrPO结果 |
|                         | 语法     | public void insert(FilmPO po)    |
| FilmDataService.insert | 前置条件 | 同样ID的po在Mapper中不存在        |
|                         | 后置条件 | 在数据库中添加一个po记录          |
|                         | 语法     | public void delete(UserPO po)    |
| FilmDataService.delete | 前置条件 | 在数据库中存在同样ID的po          |
|                         | 后置条件 | 删除一个po                        |
|                         | 语法     | public void update(FilmPO po)    |
| FilmDataService.update | 前置条件 | 数据库中存在同样ID的po            |
|                         | 后置条件 | 更新一个po                        |
|                         | 语法     | public init()                     |
| FilmDataService.init   | 前置条件 | 无                                |
|                         | 后置条件 | 初始化持久数据库                  |
|                         | 语法     | public void finish()              |
| FilmDataService.finish | 前置条件 | 无                                |
|                         | 后置条件 | 结束持久化数据库的使用            |
##### 5.4.2.3OrderDataService模块的接口规范如下表所示

​                                                                    表5-4-2-3 OrderDataService模块的接口规范  
...
##### 5.4.2.4FilmArrangeDataService模块的接口规范如下表所示

​                                                                    表5-4-2-4 OrderDataService模块的接口规范  
...
##### 5.4.2.5VIPcardDataService模块的接口规范如下表所示

   表5-4-2-5 VIPcardDataService模块的接口规范  
...
##### 5.4.2.6SalesPromotionDataService模块的接口规范如下表所示

    表5-4-2-6 SalesPromotionDataService模块的接口规范  
